...
- **BR-16 (DIREVISI)**: Nama sekolah siswa disimpan sebagai **teks bebas** (`users.nama_sekolah`), bukan lagi relasi ke tabel `Sekolah` terstruktur — terlalu banyak sekolah di Indonesia untuk dipopulate satu-satu. **Konsekuensi**: data akreditasi/kuota sekolah tidak lagi otomatis tersedia (fitur ini dihapus dari scope, sebelumnya toh tidak pernah benar-benar dipakai di rumus Keketatan/Peluang manapun). Yang tetap wajib terstruktur (bukan teks bebas) adalah **Provinsi** (`users.provinsi_id`, dropdown ~34 provinsi) — karena ini dasar validasi BR-28.
- **BR-28 (DIREVISI — aturan resmi Kemendikbudristek SNBP 2026)**: Siswa hanya boleh memilih **maksimal 2 program studi** untuk SNBP (bukan 4). Kalau memilih 2, **minimal satu wajib berada di PTN dengan provinsi yang sama dengan `users.provinsi_id` milik siswa** (diisi langsung saat onboarding/Profil, BUKAN lagi diturunkan dari sekolah→kota→provinsi). Ini aturan eksternal resmi (bukan keputusan bisnis Dimentoring) — sistem wajib menegakkan ini secara otomatis, dan **PRD/desain harus di-update ulang setiap kali SNPMB mengubah aturan** di tahun-tahun berikutnya (jangan asumsikan aturan ini permanen selamanya).
...
- **ReferralRewardConfig (baru)** — id (singleton, selalu `1`), poin_referrer, poin_referee, rupiah_per_100_poin, updated_at. Besaran poin reward referral (FR-R6: "ditentukan Admin") — diedit Admin lewat Table Editor tanpa perlu developer redeploy kode, bukan hardcode di aplikasi.
