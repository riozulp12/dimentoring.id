...
import { getSiswaBinaanGrouped } from "@/lib/mentor/getSiswaBinaanData";
import PageTitle from "@/components/dashboard/PageTitle";
import SiswaBinaanList from "@/components/mentor/SiswaBinaanList";

/** "Siswa Saya" (Mentor) — PRD Bagian 7.5. BR-7: cuma siswa di kelas yang di-assign Admin. */
export default async function SiswaBinaanPage({
  ...
  // BR-27: fitur mengajar (termasuk Siswa Saya) terkunci selama Pending.
  if ((await getMentorRoleStatus(session.userId)) !== "active") {
    redirect("/dashboard/mentor");
  }

  const groups = await getSiswaBinaanGrouped(session.userId);

  return (
    <>
      <PageTitle value="Siswa Saya" />
